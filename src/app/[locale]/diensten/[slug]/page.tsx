...
export async function generateMetadata({ params }: ServicePageProps): Promise<Metadata> {
  ...
  return {
    ...
    description,
    alternates: {
      canonical: `${SITE_CONFIG.url}/${locale}/diensten/${slug}`,
    },
    openGraph: {
...
export default async function ServiceDetailPage({ params }: ServicePageProps) {
  ...
  const pricing = PRICING[slug as ServiceSlug];
  const rawFeatures = SERVICE_FEATURES[slug as ServiceSlug] ?? [];
  ...
  const relatedProjects = projects
    .filter((p) => p.type === (slug === "metselwerk" ? "nieuwbouw" : slug === "gevelrenovatie" ? "renovatie" : slug === "monumentale-restauratie" ? "restauratie" : slug === "isolatie" ? "isolatie" : "renovatie"))
    .slice(0, 3);
      ...
      <div className="border-b border-[var(--color-border)] bg-[var(--color-surface)] pt-24">
        <div className="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
          <Breadcrumbs
            locale={locale}
            items={[
              { label: isNl ? "Diensten" : "Services", href: `/${locale}/diensten` },
      ...
      <section className="relative overflow-hidden bg-[var(--color-background)]">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="-mt-32 relative z-10 pb-16">
            <div className="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-2xl bg-[var(--color-accent-muted)] border border-[var(--color-accent)]/20">
              ...
              <ScrollReveal direction="left" delay={0.2}>
                <div className="rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)] p-6 text-center">
                  ...
                  <a
                    ...
                    className="block text-xl font-bold text-[var(--color-accent)] hover:text-[var(--color-accent-hover)] transition-colors"
                  >
